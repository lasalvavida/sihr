...
function compress(string, buffer) {
  ...
  if (bytePairIndices.length > numEncodeBytes) {
    ...
    bytePairIndices.slice(0, numEncodeBytes);
  }
  for (bytePairIndex in bytePairs) {
    if (bytePairs.hasOwnProperty(bytePairIndex)) {
      if (bytePairs[bytePairIndex] > 3) {
        while (usedBytes[encodeByte] && encodeByte < 256) {
          encodeByte++;
        }
        if (encodeByte === 256) {
          break;
